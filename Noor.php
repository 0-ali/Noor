...
protected function result($Data){
   ...
   $looking = preg_match($preg['answer'], $answer, $result);
    ...
    if($looking){
     ...
     $_array  = array( 'answer' => htmlspecialchars(strip_tags($result[0])) , 'confidence' => $confidence[1]."%" , 'title' => $title[1]);
     // return result . 
     $_return = $_array;
...
protected function MuiltfetchAnswer($url){
    $Query = $this->cURL($url);    
    preg_match_all('/<a href=" (.*?)" class="display_link" rel="nofollow">/s', $Query, $urls);
    if(is_array($urls)){
    $n = count($urls[1]);
    var_dump($url[1]);
   for ($i=0; $i < $n ; $i++) { 
